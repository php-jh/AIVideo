...
from PyQt5.QtWidgets import (
    ...
    QScrollArea, QFileDialog, QMessageBox, QFrame, QSizePolicy, QComboBox,
)
...
COMPACT_CARD_HEIGHT = THUMB_H + 200
...
class CharacterRefCard(QFrame):
    ...
    def __init__(
        self, name: str = "", ref_path: str = "", tts_voice: str = "", parent=None,
    ...
    def _build_ui(self):
        ...
        self.voice_combo.addItem("配音：自动(按性别)", "")
        for vid, desc in VoiceGenerator.VOICE_MAP.items():
            short = desc.split("（")[0] if "（" in desc else desc[:8]
            self.voice_combo.addItem(f"{short}", vid)
        self.voice_combo.setMinimumHeight(28 if compact else 32)
        self.voice_combo.setStyleSheet(
            "QComboBox { background: #1a1a2e; color: #e6e6e6; border: 1px solid #3a3a5c; "
            "border-radius: 4px; padding: 2px 6px; font-size: 11px; }"
        )
        self.voice_combo.currentIndexChanged.connect(lambda: self.changed.emit())
...
class CharacterRefsPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        if self._large_cards:
            ...
            hint = QLabel(
                "为各角色上传参考图；下拉框可指定 Edge TTS 配音音色（不选则按性别自动分配）"
            )
    ...
    def add_card(
        self, name: str = "", ref_path: str = "", tts_voice: str = "", insert_at: int = -1,
    ) -> CharacterRefCard:
        card = CharacterRefCard(
            name, ref_path, tts_voice, self.cards_host,
    ...
    def merge_from_script(self, script: Optional[dict]):
        ...
        voices_by_name: Dict[str, str] = {}
        for card in self._cards:
            n = card.get_name()
            p = card.get_reference_path()
            if n and p:
                refs_by_name[n] = p
            v = card.get_tts_voice()
            if n and v:
                voices_by_name[n] = v
