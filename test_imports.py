...
def test_pipeline_resume():
    ...
    try:
        ...
        script = {
            "scenes": [
                {"scene_number": 1, "narration": "test"},
                {"scene_number": 2, "narration": "test2"},
            ]
        }
        with tempfile.TemporaryDirectory() as tmp:
            st = analyze_pipeline(script, tmp)
            assert st.total_scenes == 2
            assert st.next_step == "images"
            assert st.images_done == 0
        return True
    except Exception as e:
        print(f"pipeline_resume 测试失败: {e}")
        return False
...
def main():
    ...
    print("\n3. 测试断点续跑:")
    if test_pipeline_resume():
        print("[OK] pipeline_resume 测试通过")
    else:
        print("[FAIL] pipeline_resume 测试失败")
        return
